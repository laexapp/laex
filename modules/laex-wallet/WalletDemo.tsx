    ...
    <div className="lw-app">
      ...
      <main className={`lw-content lw-screen-${screen} ${showNav ? "lw-with-nav" : ""}`}>
        ...
        {screen === "home" && <section>
          ...
          {extras.network === "9580" && <button className="lw-menu-row" onClick={()=>go("tokens")}><div><b>OMDB</b><small>Moneda nativa de OMDBlockchain · Configurada</small></div><span>0 OMDB</span></button>}
          ...
          {extras.tokens.filter(t=>t.network===extras.network).map(t=><button className="lw-menu-row" key={t.address} onClick={()=>go("tokens")}><div><b>{t.symbol}</b><small>{t.name} · {t.preset ? "BNB Chain · Preconfigurado" : "Datos manuales"}</small></div><span>0</span></button>)}
