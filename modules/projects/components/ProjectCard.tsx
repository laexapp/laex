...
export default function ProjectCard({ project, index = 0 }: Props) {
  ...
  return (
    <article style={style} className="group relative flex min-h-[470px] flex-col overflow-hidden rounded-[28px] border border-white/[0.09] bg-[#071018]/80 p-5 shadow-[0_24px_65px_rgba(0,0,0,.34)] transition duration-500 hover:-translate-y-1.5 hover:border-cyan-300/25 hover:shadow-[0_34px_85px_rgba(0,0,0,.52)]">
      ...
      <div className="relative h-48 overflow-hidden rounded-[20px] border border-white/[0.07] bg-black/20">
        <Image src={project.banner} alt="" fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover opacity-60 transition duration-700 group-hover:scale-105 group-hover:opacity-75" />
      ...
      <div className="relative z-10 mt-6 flex flex-1 flex-col">
        ...
        <Link href={`/proyectos/${project.id}`} className="mt-auto flex min-h-12 items-center justify-between border-t border-white/[0.07] pt-5 text-[10px] font-bold uppercase tracking-[0.15em] text-slate-400 transition hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/70" aria-label={`Explorar ${project.name}`}>
          Explorar proyecto
