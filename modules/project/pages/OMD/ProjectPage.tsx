...
function ReferralPanel({ user }: { user: ReturnType<typeof useCurrentUser> }) {
  return <section id="conexion" className="mt-5 overflow-hidden rounded-[2rem] border border-violet-300/18 bg-[linear-gradient(135deg,rgba(13,11,31,.96),rgba(4,12,22,.98))] p-6 sm:p-8"><div className="grid gap-8 lg:grid-cols-[1fr_.85fr] lg:items-center"><div><div className="flex items-center gap-3 text-violet-300"><LockKeyhole size={18} /><span className="text-[10px] font-black uppercase tracking-[.2em]">Conexión personal y aislada</span></div><h2 className="mt-4 text-3xl font-bold tracking-[-.04em]">Tu línea de referido pertenece a tu usuario.</h2><p className="mt-4 max-w-2xl text-sm leading-7 text-slate-400">LAEX separa la configuración global del proveedor del identificador personal. Nunca utiliza el código de otra persona como alternativa.</p><div className="mt-5 flex flex-wrap gap-2"><Evidence tone="violet">Proveedor configurable</Evidence><Evidence tone="cyan">Código por usuario</Evidence><Evidence tone="green">Aislamiento por UID</Evidence></div></div><div className="rounded-2xl border border-white/[.08] bg-black/20 p-5"><p className="text-[10px] font-bold uppercase tracking-[.16em] text-slate-600">Sesión LAEX</p><div className="mt-3 flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-xl border border-violet-300/20 bg-violet-300/[.07] text-violet-200"><UserRound size={19} /></span><div><strong className="block text-sm">{user?.fullName || "Usuario no autenticado"}</strong><span className="text-[10px] text-slate-500">{user ? "Integración OMD Pool no configurada" : "Inicia sesión para ver tu integración"}</span></div></div><button disabled className="mt-5 flex min-h-12 w-full cursor-not-allowed items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[.03] px-4 text-[10px] font-black uppercase tracking-[.12em] text-slate-500"><LockKeyhole size={13} />{user ? "Configuración protegida · próxima activación" : "Iniciar sesión para configurar"}</button><p className="mt-3 text-[10px] leading-5 text-slate-600">La escritura permanece bloqueada en este Preview hasta versionar y desplegar reglas Firestore por UID. No se expone ni se reutiliza Luis12.</p></div></div></section>;
}
