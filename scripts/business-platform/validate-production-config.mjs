...
if (!["true", "false"].includes(process.env.LAEX_MULTIDOMAIN_ROUTING_ENABLED ?? "")) {
  invalid.push("LAEX_MULTIDOMAIN_ROUTING_ENABLED");
}
...
if (missing.length || invalid.length) {
  ...
  console.log(JSON.stringify({
    ready: true,
    requiredVariablesPresent: required.length,
    indexingEnabled: false,
    multidomainRoutingEnabled: process.env.LAEX_MULTIDOMAIN_ROUTING_ENABLED === "true",
    directDatabaseUrlRequiredByRuntime: false,
  }));
}
